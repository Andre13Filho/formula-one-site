  ...
  try {
    ...
    await Promise.all(
      meetings.map(async (m) => {
        ...
        const info = driverMap[winnerNumber];
        const winnerName = info ? info.full_name : (winnerNumber ? `#${winnerNumber}` : "-");
    ...
    rows
      ...
      .forEach((row) => {
        ...
        yearCell.setAttribute("data-label", "Ano");

        const nameCell = document.createElement("td");
        nameCell.textContent = row.name;
        nameCell.setAttribute("data-label", "Grand Prix");

        const winnerCell = document.createElement("td");
        winnerCell.textContent = row.winner;
        winnerCell.setAttribute("data-label", "Vencedor");
